...
public:
    ...
    c74::min::attribute<c74::min::symbol> config{this, "config", "",
        ...
        c74::min::setter{[this](const c74::min::atoms &args, int) -> c74::min::atoms {
            ...
            if(bbb::dmx::maxutil::should_mark_explicit_symbol_override(args, applying_setup_, suppress_config_attribute_load_)) {
...
private:
    ...
    void apply_setup_values(const bbb::dmx::dmx_setup_values &values, const std::string &base_directory) {
        ...
        if(values.config.has_value() && !config_attribute_overridden_) {
            const std::string resolved_path{bbb::dmx::maxutil::setup_relative_path(base_directory, values.config.value())};
            bbb::dmx::maxutil::apply_setup_symbol_path(config, suppress_config_attribute_load_, config_path_value_, resolved_path);
            load_config(config_path_value_);
        }
