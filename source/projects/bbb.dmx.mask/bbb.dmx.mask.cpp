...
public:
    ...
    MIN_TAGS{"dmx, lighting, mask, mute, hold, universe"};
    MIN_AUTHOR{"2bit"};
    MIN_RELATED{"bbb.dmx.curve, bbb.dmx.safety, bbb.dmx.merge"};

    c74::min::inlet<> input{this, "(list/universe/channel/channels/read/bang) DMX mask input"};
    ...
    c74::min::message<> mute_message{this, "mute", "mute universe start count",
        MIN_FUNCTION {
            if(args.size() < 3 || !bbb::dmx::maxutil::finite_atoms(args, 0, 3)) {
                report_error("mute requires universe start count");
                return {};
            }
    ...
    c74::min::message<> hold_message{this, "hold", "hold universe start count",
        MIN_FUNCTION {
            if(args.size() < 3 || !bbb::dmx::maxutil::finite_atoms(args, 0, 3)) {
                report_error("hold requires universe start count");
                return {};
            }
    ...
    c74::min::message<> allow_message{this, "allow", "allow universe start count",
        MIN_FUNCTION {
            if(args.size() < 3 || !bbb::dmx::maxutil::finite_atoms(args, 0, 3)) {
                report_error("allow requires universe start count");
                return {};
            }
    ...
    c74::min::message<> force_message{this, "force", "force universe start count value",
        MIN_FUNCTION {
            if(args.size() < 4 || !bbb::dmx::maxutil::finite_atoms(args, 0, 4)) {
                report_error("force requires universe start count value");
                return {};
            }
