...
- `@color_wheel_fallback` — optional semantic color fallback. Default `0` means RGB requests never touch a color wheel; `1` maps non-RGB/CMY fixtures to the nearest wheel slot when fixture metadata contains usable `wheels`/`ranges`.

Load / inspect messages:

```max
read patches/example.json
readgroups groups/example.groups.json
reload
dump
clear
reset
...
trackall 0. 0. 1.5
trackgroup front 0. 0. 1.5
trackrel spot_01 0. -1. 0.
trackallrel 0. -1. 0.
trackgrouprel front 0. -1. 0.
trackreset [spot_01]
```
...
```max
[bbb.dmx.patchcheck @patch patches/example.json @groups groups/example.groups.json]
```
