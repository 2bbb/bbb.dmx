...
`tools/bbb-dmx-convert` also builds `bbb-dmx-lint`, a Node.js validator for `bbb.dmx` JSON files. It runs the repository JSON Schemas and semantic checks that require cross-file context, such as patch/profile resolution, duplicate fixture ids, fixture footprint overflow, channel collisions, duplicate channel offsets/keys, and parameter references to missing channels.

```sh
cd tools/bbb-dmx-convert
npm install
npm run build
node dist/lint.js ../../patches/example.json ../../scenes/example.json
node dist/lint.js ../../patches/example.json --fixture-dir ../../fixtures --strict
node dist/lint.js ../../patches/example.json --json
```
