...
- GDTF `Wheels/Wheel/Slot` data becomes profile-level `wheels[]` metadata. Color slots keep approximate `rgb` or CIE `cie_xyY` data when source data exposes it.
- GDTF-style `ChannelFunction` / `ChannelSet` value tables become parameter `ranges` when a channel has multiple functions or 2+ named sets. This covers shutter open/closed/strobe/pulse/random, color/gobo wheel slots, prism/frost/iris modes, and similar stepped parameters. `DMXFrom` values are scaled into the parameter domain (`u8`, `u16`, or `u24`), `to` is inferred from the next start value, labels are preserved, `ChannelFunction/@Wheel` becomes `parameter.wheel`, `ChannelSet/@WheelSlotIndex` becomes `range.wheel_slot`, and finite `PhysicalFrom` / `PhysicalTo` values are emitted as `physical_from` / `physical_to`.
